...
function getLinkedGemId(order) {
  return (
    order?.gemId ||
    order?.inventoryGem ||
    order?.selections?.gemId ||
    order?.selections?.gem?._id ||
    order?.selections?.gem?.id ||
    null
  );
}
// === atomic status setter with guard & tiny debug log ===
async function setGemStatusAtomic(gemId, nextStatus, onlyIfCurrentNotIn = []) {
  if (!gemId) return;

  const query = { _id: gemId };
  if (onlyIfCurrentNotIn.length) {
    query.status = { $nin: onlyIfCurrentNotIn };
  }

  const update = { $set: { status: nextStatus } };
  if (nextStatus === "out_of_stock") {
    update.$set.isActive = false; // hide from listing
  }

  const result = await Gem.updateOne(query, update);
  if (result.modifiedCount === 0) {
    console.warn("Gem status not modified", {
      gemId: String(gemId),
      nextStatus,
      onlyIfCurrentNotIn,
...
async function markGemSoldForOrder(order) {
 try {
    const gemId = getLinkedGemId(order);
    if (!gemId) return;

    // already guarded by atomic query; no need to load+save
    await setGemStatusAtomic(gemId, 'out_of_stock', ['out_of_stock']); // guard: skip if already OOS
  } catch (e) {
    console.warn('Could not mark gem out_of_stock for order', order?._id, e?.message || e);
  }
}
...
async function markGemReservedForOrder(order) {
   try {
    ...
    // only reserve if not already reserved/sold/out_of_stock
    await setGemStatusAtomic(gemId, 'reserved', ['reserved', 'sold', 'out_of_stock']);
  } catch (e) {
...
exports.checkout = async (req, res, next) => {
  try {
    ...
    try {
      let w = await Winner.findOne({
        auctionCode: order.orderNo,
        user: order.buyerId,
      });
      if (!w) {
        // Fallback: find auction by its human code then winner by auction _id
        const a = await Auction.findOne({ auctionId: order.orderNo }).select(
          "_id"
        );
        if (a)
          w = await Winner.findOne({ auction: a._id, user: order.buyerId });
      }
      if (w) {
...
exports.checkoutFromGem = async (req, res, next) => {
  ...
  const _setGemStatusAtomic = async (gemId, next, notIn=[]) => {
  const q = { _id: gemId };
  if (notIn.length) q.status = { $nin: notIn };
  const u = { $set: { status: next } };
  if (next === 'out_of_stock') u.$set.isActive = false;
  const r = await Gem.updateOne(q, u);
  if (r.modifiedCount === 0) console.warn('Gem status not modified (fromGem)', { gemId: String(gemId), next });
};

  // helpers reused from the standard checkout above
  const finalizeGemSoldForOrder = async (order) => {
    try {
    const gemId = _getLinkedGemId(order);
    if (!gemId) return;
    await _setGemStatusAtomic(gemId, 'out_of_stock', ['out_of_stock']);
  } catch (e) {
  ...
  const ensureGemReservedForOrder = async (order) => {
    ...
    const gemId = _getLinkedGemId(order);
    if (!gemId) return;
    await _setGemStatusAtomic(gemId, 'reserved', ['reserved', 'sold', 'out_of_stock']);
  } catch (e) {
  ...
  try {
    ...
    try {
      ...
      return res.status(400).json({ ok: false, error: "INVALID_GEM" });
    }
    if (!gem)
      return res.status(404).json({ ok: false, error: "GEM_NOT_FOUND" });
    const s = String(gem.status || "")
      .toLowerCase()
      .replace(/\s+/g, "_");
...
exports.checkoutCustom = async (req, res, next) => {
  try {
    ...
    // Validate selections
    const missing = [type, shape, weight, grade, polish, symmetry].some(
      (v) => v === undefined || v === null || v === ""
    );
    if (missing)
      return res.status(400).json({ ok: false, error: "MISSING_SELECTIONS" });
    ...
    const orderNo = `CORD-${new Date().getFullYear()}-${nanoid(
      6
    ).toUpperCase()}`;
  ...
  try {
    ...
    const [items, total] = await Promise.all([
      Payment.find(q)
        ...
        .populate({
          path: "orderId",
          select: "title selections pricing status estimatedFinishDate",
        }),
  ...
  try {
    ...
    const [items, total] = await Promise.all([
      Payment.find(q)
        ...
        .populate({
          path: "orderId",
          select: "title pricing status estimatedFinishDate",
        }),
      Payment.countDocuments(q),
    ]);
...
exports.markBankPaid = async (req, res, next) => {
  try {
    ...
    if (p.payment.method !== "bank") {
      return res
        .status(400)
        .json({ ok: false, message: "Only bank payments can be marked paid" });
    }
    ...
    try {
      const w = await Winner.findOne({
        auctionCode: p.orderNo,
        user: p.buyerId,
      });
      if (w) {
        w.purchaseStatus = "paid";
  ...
  try {
    ...
    if (p.payment?.method !== "bank") {
      return res
        ...
        .json({
          ok: false,
          message: "Only bank payments can be manually changed",
        });
    }
    ...
    if (cur === "paid") {
      return res
        .status(400)
        .json({ ok: false, message: "Paid payments are final" });
    }
    if (cur === "cancelled") {
      return res
        .status(400)
        .json({ ok: false, message: "Cancelled payments are final" });
    }
    ...
    async function syncRelated(orderStatus) {
      ...
      try {
        ...
        if (!w) {
          const a = await Auction.findOne({ auctionId: p.orderNo }).select(
            "_id"
          );
  ...
  try {
    ...
    if (p.payment?.method !== "card") {
      return res
        ...
        .json({
          ok: false,
          message: "Only card payments can have saved cards",
        });
    }
    if (!p.payment?.card) {
      return res
        .status(400)
        .json({ ok: false, message: "No saved card on this payment" });
    }
