...
exports.summary = async (req, res, next) => {
  try {
    ...
    let totalRevenue = 0, paidCount = 0, pendingCount = 0, cancelledCount = 0, currency = 'USD';
    for (const r of rows) {
      if (r._id === 'paid') {
    ...
    res.json({ ok: true, totalRevenue, paidCount, pendingCount, cancelledCount, currency, year });
  } catch (err) {
    next(err);
  }
};
...
exports.monthlyRevenue = async (req, res, next) => {
  try {
    ...
    const rows = await Payment.aggregate([
      { $match: { 'payment.status': 'paid', createdAt: { $gte: start, $lt: end } } },
      {
        $group: {
...
exports.categoryBreakdown = async (req, res, next) => {
  try {
    ...
    const buckets = { Sapphires: 0, Rubies: 0, Emeralds: 0, Diamonds: 0, Others: 0 };
    const re = {
      Sapphires: /sapphir/i,
      Rubies: /ruby/i,
      Emeralds: /emerald/i,
