...
export default function AdminDashboard() {
  ...
  const [deletingId, setDeletingId] = useState(null); // ⬅️ track which row is deleting
  ...
  const handleDeleteUser = async (id) => {
    ...
    try {
      ...
      // refresh the small preview list
            ...
            datasets: [
              {
                ...
                data: [
                  28500, 31200, 29800, 33400, 36700, 41200, 45600, 49800, 52300,
                  56800,
                ],
                borderColor: "#d4af37",
                backgroundColor: grad,
                ...
                <tbody>
                  {complaintsRows.map((c) => (
                    <tr key={c._id}>
                      ...
                      <td>
                        {c.userName || c.user?.fullName || c.user?.email || "—"}
                      </td>
                      <td>{c.subject}</td>
                      <td>{c.priority}</td>
                      <td>
