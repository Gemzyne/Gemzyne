...
// Page-scoped styles (seller look-and-feel)
...
// only website-related?
const isWebsite = (fb) => {
  const cat1 = (fb.complaintCategory || "").toString().toLowerCase();
  const cats = Array.isArray(fb.categories) ? fb.categories.map(c => (c || "").toString().toLowerCase()) : [];
  return cat1.includes("website") || cats.some(c => c.includes("website"));
};

export default function AdminFeedbackHub() {
  const [view, setView] = useState("complaints");       // "reviews" | "complaints"
  const [statusFilter, setStatusFilter] = useState("all"); // for complaints: all|pending|resolved
  ...
  // Fetch all feedback for admins (we'll filter to website-only client-side)
  const loadAll = async () => {
    try {
      setLoading(true);
      setError("");
  ...
  // Actions (same semantics as seller page)
  ...
  // Website-only filters
  const allReviews = useMemo(() => items.filter((i) => i.type === "review" && isWebsite(i)), [items]);
  const allComplaints = useMemo(() => items.filter((i) => i.type === "complaint" && isWebsite(i)), [items]);

  // Derive KPI for sidecard (only website data)
  const { responseRate, resolutionRate } = useMemo(() => {
    const total = allComplaints.length;
    if (!total) return { responseRate: 0, resolutionRate: 0 };
    const responded = allComplaints.filter((c) => c?.adminReply?.text?.trim()).length;
    const resolved  = allComplaints.filter((c) => String(c.status || "").toLowerCase() === "resolved").length;
    return {
      ...
      <div className="adfb-shell">
        ...
        <main className="adfb-main">
          ...
          <section className="adfb-page-header">
            <h1>Website Feedback Management</h1>
            <p>Admin view • Website-related complaints & feedback only</p>
          </section>

          {/* Filters (status for complaints only) */}
          ...
          <div className="adfb-content">
            ...
            <div className="adfb-card">
              <div className="adfb-card-header">
                <h3>{view === "reviews" ? "Website Feedback (Reviews)" : "Website Complaints"}</h3>
                {!loading && (
                  <span>
                    Showing {shown.length} of {view === "reviews" ? allReviews.length : allComplaints.length} {view}
                  </span>
                )}
              </div>
              ...
              {!loading && !error && shown.map((f) => {
                ...
                return (
                  <div className={`adfb-item ${rowClass}`} key={f._id}>
                    ...
                    {/* Admin actions (same UX as seller) */}
            ...
            <aside className="adfb-sidecard">
              <div className="adfb-sidecard__title">Website Feedback Overview</div>
