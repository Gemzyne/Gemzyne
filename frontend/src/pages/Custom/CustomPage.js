...
export default function CustomPage() {
  ...
  const basePrice = useMemo(
    () => GEM_TYPES.find((g) => g.key === type)?.price || 0,
    [type]
  );
  const shapePrice = useMemo(
    () => SHAPES.find((s) => s.key === shape)?.price || 0,
    [shape]
  );
  const weightPrice = useMemo(
    () => (weight && weight > 1 ? Math.round((weight - 1) * 1000) : 0),
    [weight]
  );
  const gradePrice = useMemo(
    () =>
      grade === "premium"
        ? 1500
        : grade === "excellent"
  ...
  const polishPrice = useMemo(
    () => (polish === "excellent" ? 300 : polish === "very-good" ? 150 : 0),
    [polish]
  );
  const symmetryPrice = useMemo(
    () => (symmetry === "excellent" ? 250 : symmetry === "very-good" ? 100 : 0),
    [symmetry]
  );
  const totalPrice =
  ...
  const allSelected =
    !!type && !!shape && weight != null && !!grade && !!polish && !!symmetry;
    ...
    <>
      ...
      <div className="customization-container">
        <div className="customization-header">
          ...
          <p>
            Customize every aspect of your gemstone to create a truly unique
              ...
              {tab === "shape" && (
                <section className="customization-section active" id="shape-section">
                  ...
                  <p>
                    ...
                    character. Choose what speaks to you.
                  </p>
                  <div className="option-grid">
                    {SHAPES.map((s) => (
                      <div
                    ...
                    <div className="weight-grid" id="weight-grid">
                      {WEIGHTS.map((w) => (
                        <button
                          type="button"
                          key={w}
                          className={`chip ${weight === w ? "selected" : ""}`}
                          onClick={() => setWeight(w)}
                        >
                ...
                <section className="customization-section active" id="review-section">
                  ...
                  <div className="text-preview">
                    ...
                    <div className="preview-details">
                      ...
                      <div className="preview-item">
                        <div className="preview-label">Grade</div>
                        <div className="preview-value">{grade || "—"}</div>
                      </div>
                      <div className="preview-item">
                        <div className="preview-label">Polish</div>
                        <div className="preview-value">{polish || "—"}</div>
                      </div>
                      <div className="preview-item">
                        <div className="preview-label">Symmetry</div>
                        <div className="preview-value">{symmetry || "—"}</div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="order-summary">
              ...
              <div className="summary-detail">
                <div className="detail-item">
                  <span className="detail-label">Gem Type:</span>
                  <span className="detail-value">{type ? cap(type) : "—"}</span>
                </div>
                <div className="detail-item">
                ...
                <div className="detail-item">
                  ...
                  <span className="detail-value">
                    {weight != null
                      ? `${weight.toFixed(1).replace(/\.0$/, "")} carats`
                      : "—"}
                  </span>
                </div>
                <div className="detail-item">
                ...
                <div className="detail-item">
                  <span className="detail-label">Polish:</span>
                  <span className="detail-value">{polish || "—"}</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">Symmetry:</span>
                  <span className="detail-value">{symmetry || "—"}</span>
                </div>
              </div>

              <div className="order-item">
                <span className="item-name">Base Price</span>
                <span className="item-price">{money(basePrice)}</span>
              </div>
              <div className="order-item">
                <span className="item-name">Shape</span>
                <span className="item-price">{money(shapePrice)}</span>
              </div>
              <div className="order-item">
                <span className="item-name">Weight</span>
                <span className="item-price">{money(weightPrice)}</span>
              </div>
              <div className="order-item">
                <span className="item-name">Grade</span>
                <span className="item-price">{money(gradePrice)}</span>
              </div>
              <div className="order-item">
                <span className="item-name">Polish</span>
                <span className="item-price">{money(polishPrice)}</span>
              </div>
              <div className="order-item">
                <span className="item-name">Symmetry</span>
                <span className="item-price">{money(symmetryPrice)}</span>
              </div>
